...
class BotFrameworkInput(InputChannel):
    ...
    def _update_cached_jwk_keys(self) -> None:
        ...
        req = requests.get(MICROSOFT_OPEN_ID_URI)
    ...
    def blueprint(
        ...
        async def webhook(request: Request) -> HTTPResponse:
            ...
            if datetime.datetime.now() - self.jwt_update_time > datetime.timedelta(
                days=1
            ):
                try:
                    self._update_cached_jwk_keys()
            ...
            auth_match = BEARER_REGEX.match(request.headers.get("Authorization"))
            if not auth_match:
            ...
            try:
                ...
                logger.exception(error, exc_info=True)
                return response.text(
                    "Could not validate JWT token.", status=HTTPStatus.BAD_REQUEST
                )
